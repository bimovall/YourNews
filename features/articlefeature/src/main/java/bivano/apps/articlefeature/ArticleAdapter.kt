...
class ArticleAdapter : PagedListAdapter<Article, RecyclerView.ViewHolder>(ArticleDiffCallback) {

    var onItemClick: ((Article) -> Unit)? = null

    private var networkState : Result<List<Article>>? = null
    ...
    private fun hasLoadMore(): Boolean {
        return networkState != null && networkState !is Result.Success
    }

    fun setNetworkState(state: Result<List<Article>>) {
    ...
    inner class LoaderViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        fun bindState(result: Result<List<Article>>) {
